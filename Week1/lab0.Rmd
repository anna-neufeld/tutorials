---
title: "Lab 1: Working with RStudio and RMarkdown"
output:
  html_document:
    css: ../lab.css
    highlight: pygments
...
This lab will require you to run some very basic commands in R. You will explore the various windows in the RStudio interface. You will also practice recording your answers in an RMarkdown lab report and submitting a Canvas check-in quiz. 

## Getting Started

Hopefully you were able to download R and RStudio by following the Prelab instructions. If so, go ahead and launch RStudio. You should see a window that looks like the image 
shown below.
...
- **oilabs**: additional OpenIntro resources, including documentation for the datasets for the labs. 
...
inspecting the *Packages* tab in the lower right panel of RStudio. If you do not see ``tidyverse``, ``oilabs``, and ``openintro``  in this panel, it means that you need to run the following code from the pre-lab. 
...
Once the template is open, you should see the following:
![](template.png)
...
1. **Quiz Question:** Use R to compute $128 \times 42 - \frac{145}{9} + 1.7^4$. Round your answer to 1 decimal place. 
...
Exercise 1 is marked as a **Quiz Question**. In this class, that means that you will not only record the answer in your R Markdown document, but you will also submit the answer on the corresponding Canvas quiz. The Canvas quiz is auto-graded and you can see immediately if your answer is correct. The Canvas quiz questions are a nice way to make sure you are on the right track as you move through a lab. This week, there are only 5 questions on the lab and all 5 are quiz questions. Sometimes, you will have a lab with many questions, only a subset of which are quiz questions. 
...
2. **Quiz Question**: Use R to compute the value $\sqrt{1127}$. Upload this value to Canvas (rounded to two decimal places) and save this value to a variable `X`. 
...
3. **Quiz Question**. Use R to compute the value $\sqrt{1+2+3+4}$. Create a variable `statistics_is_fun` that stores this value. 

4. **Quiz Question**: Using R, compute $\frac{X + statistics\_is\_fun}{X}$. 

Don't forget to upload your answers to Canvas to see if they are correct. Canvas has a built-in rounding tolerance, so you will not lose points for rounding to the wrong nummber of decimal places unless you are explicitly told to round in a certain way. Also- be sure to knit your document before moving on. Is everything working okay? 
...
5. **Quiz Question:** Resolve the issue that you ran into above so that exercise 5 correctly saves a variable called `z`, a variable called `y`, and prints the difference. Upload the correct difference to Canvas. 
