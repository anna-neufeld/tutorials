...
In the next few minutes, modify your example document so that it looks something like this one: 
...
When an R Markdown document is knitting, it only has access to the variables that are created inside of the R Markdown document itself. The R Markdown document does not get to use the environment in the upper right hand corner of your screen while it is knitting. The R Markdown document only gets to see the variables that are defined inside of the document. To illustrate this concept, delete the line ``x <- 5+7`` from your sample document. 
...
Can you still run the chunk locally? What happens if you try to knit your document? 
...
Over the weekend, 67 Stat 202 students filled out the "getting to know you" survey. Google forms lets me export the survey responses into a google sheet / Excel spreadsheet. I removed the names, pronouns, and email addresses from the spreadsheet, and I also removed the answers to the "free response" questions (e.g. "Why are you taking this class"). Finally, I saved the resulting spreadsheet as a `.csv`. 

I posted the anonymized data on GLOW as a `.csv` file, and I also posted it on my github page. There are two ways that you can load the data. The easiest way would be to run this code:

```{r}
read.csv("https://anna-neufeld.github.io/stat202_tutorials/Week1/form_data_anonymous.csv")
```
...
- I think that this is all you need for your upcoming homework! 
</div>

### One quantitative variable

It seems as though "sleep_hours_summer" should be a quantitative variable, and the simplest possible summary we could make of a quantitative variable would be the mean!  how many hours do you sleep, on average, over the summer. However, let's look at the data.

```{r}
unique(form_data_anonymous$sleep_hours_summer)
```

Uh oh ...  because not everyone put in a number, R is treating this as a categorical variable! This is really a bummer. There are a few teachable moments here.

  + Design your surveys well! I let this happen by giving you a free response question.
  + You now have a choice to make as a researcher about how to deal with this. 
  + Here, we are going to do the easy and yet sometimes *wrong* choice. 
