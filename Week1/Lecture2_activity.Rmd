...
- Load our first dataset into R: use this to review basic concepts about exploratory data analysis. 
</div>
...
Over the weekend, 67 Stat 202 students filled out the "getting to know you" survey. Google forms lets me export the survey responses into a google sheet / Excel spreadsheet. I removed the names, pronouns, and email addresses from the spreadsheet, and I also removed the answers to the "free response" questions (e.g. "Why are you taking this class"). I changed the column names for readability. Finally, I saved the resulting spreadsheet as a `.csv`. 

I posted the anonymized data on GLOW as a `.csv` file, and I also posted it on my github page. There are two ways that you can load the data. The easiest is to run this code:

```{r}
form_data <- read.csv("https://anna-neufeld.github.io/stat202_tutorials/Week1/form_data_anonymous.csv")
```

The slightly more cumbersome (but useful!) way involves downloading the .csv document from GLOW and importing it using the ``import dataset`` button. We will go over this way later in the semester, as storing a file locally can lead to come more cumbersome knitting issues. 
...
Let's use ``pets`` which stores the recorded answers to the question "How many pets have you had or have you lived with in your life?" as our first example of a quantitative variable. 
...
The *summarize()* function is a nice way to view a numeric summary such as the mean or the median of a variable. 
...
The first is really simple, and uses "base R". What do you notice about the shape of this variable? 
...
```
Subsetting our histogram to only include seniors is simple:
...
On your own time, try making a histogram of another variable in the dataset! This may require some additional *cleaning* due to the fact that nearly every variable had issues with students entering words rather than numbers into the survey. 
...
```
When categorical variables have a natural ordering to them, it is nice to tell `R` this so that the categories will be displayed in order. Try running the following code:
...
And now try remaking the table from above. Just for fun, let's add the median number of pets, as well as the number of students, for each category. 

```{r}
form_data_clean %>% group_by(year) %>% summarize(mean(pets_numeric), median(pets_numeric), n())
```
...
In this section, we will look at the relationship between a student's favorite dining hall and whether or not they play a sport at Williams. Both of these variables are categorical, which means that some of the techniques that we learned above no longer apply!

Let's first look at the two variables individually. Note that I am going to work with ``form_data``, not ``form_data_clean``, because I no longer need to worry about the ``pets`` variable. 
...
The reason that this is convenient is that we can now do the following. Discuss with your neighbors what these numbers mean!

```{r}
form_data %>% summarize(sum(athlete), mean(athlete))
```
We can now look at the number or proportion of students who are athletes, broken down by a student's favorite dining hall. 

```{r}
form_data %>% group_by(dining_hall) %>% summarize(sum(athlete), n(),mean(athlete))
```

We can also display this visually!

```{r}
ggplot(data=form_data, aes(x=dining_hall))+geom_bar()
```
...
Do you think that there is a relationship? Is it meaningful? Discuss with your group! 

# Wrap up

Hopefully, when you work on homework 1, you will find that I went over all of the R functions that you need for the homework in class. It is my goal that this will be the case for every homework throughout the semester! 
...
The formatting and some of the introductory content in this tutorial was adopted from an OpenIntro lab. <div id="license">
