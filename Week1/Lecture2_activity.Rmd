...
I am assuming that we will run out of time to go over these sections in class, 

```{r}
ggplot(data=form_data, aes(x=dining_hall))+geom_bar()
```

Do you think that there is a relationship between favorite dining hall and whether or not you play a sport?

```{r}
table(form_data$sports)
form_data %>% group_by(dining_hall) %>% summarize(mean(sports=="Yes"))
```
...
One of the categorical variables in our dataset is ``dining_hall``, which is from the question about which dining hall students prefer. A good way to visualize this data is with a bar plot. 

```{r}
ggplot(data=form_data_anonymous, aes(x=dining_hall))+geom_bar()
```

On your own, make a bar plot of another categorical variable in the dataset. Describe what you see!
