...
Note that, inside of my ``mutate`` statement, I use both ``=`` and ``>``. Here, they serve different purposes. The ``=`` is *assigning* a value to a new variable, called ``coffee_drinker``. On the other hand, the ``>`` is *asking a question* to the data. Later, we will see that if we want to *ask a question* about if two things are equal, we need to use ``==`` instead of ``=``. 
...
```
You can always change your axis limits manually in R. By default, R sets your axis limits to capture the entire dataset.
...
**Challenge:** Using what you learned in the histogram section, can you figure out how to add axis labels and a title to your ``ggplot`` boxplot?
...
```{r}
ggplot(data=form_data, aes(x=dining))+geom_bar(fill="pink")+xlab("Favorite Dining Hall")+ylab("Number of Students")+ggtitle("My favorite plot")
```
...
We may also want to report numerical summaries of the ``height_numeric`` variable across our two groups of the ``sports`` variable. The tidyverse makes this very seamless.
...
```{r}
form_data_subset %>% filter( sports=="No") %>% summarize(mean(height_numeric), sd(height_numeric), median(height_numeric))
form_data_subset %>% filter( sports=="Yes") %>% summarize(mean(height_numeric), sd(height_numeric), median(height_numeric))
```
