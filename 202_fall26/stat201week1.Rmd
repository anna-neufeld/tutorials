...
# Exploring indvidual variables in our dataset

Datasets are usually too big to understand just by looking at the raw data. We need to go one variable at a time and produce numerical and visual summaries. We then need to look at relationships between variables. Luckily, R makes this all pretty straightforward.

<div id="boxedtext">

**Goals:**

- Go over methods for numerically or visually summarizing different types of variables. Here are some key R functions that we will learn.

- One numerical variable: ``mean()``, ``median()``, ``sd()``, ``geom_histogram()``. 
- One categorical variable: ``table()``, ``geom_bar()``. 
- One categorical and one numerical: ``group_by()``, ``summarize()``, ``geom_boxplot()``.
- Two categorical variables: continue with tables and barplots. 
- Two numerical variables: ``geom_point()``, ``cor()``.  
- Get exposed to the following functions:
