...
R is an incredibly popular programming language for data analysis. It is used by statisticians but also by scientists in biology, psychology, economics, chemistry, etc. R is fully featured and very powerful, and unlike Stata or SAS, it is *free and open source*.
This means that you can continue to use it for free after you finish
this class, and beyond. Additionally, R is *extensible*.
When a statistician develops a new method, or when a scientist decides that they need a specific tool to analyze their specific data, they (or someone who wants to use it in R) will often implement the method as a collection of functions in R called a *package*. Other individuals who want to use the method or conduct a similar type of data analysis can download the package and use these functions without needing to rewrite all of the code themselves. The Comprehensive R Archive Network, or **CRAN**, contains over 20,000 [contributed packages](https://cran.r-project.org/web/packages/). 
...
+ Many "textbook" datasets are built into packages. These are extremely easy to load. As long as you have the package installed, you just load the package and then access the relevant dataset with the ``data()`` function. This is what you will be doing on HW1! These are typically datasets that are meant for educational purposes. 
...
- Use the functions ``select()``, ``filter()``, and ``mutate()``.
</div>
...
We have a problem, which is that R has not identified ``pets" as a numerical variable. 

```{r, eval=F}
class(form_data$pets)
```

The fact that not everyone entered their number of pets as a number on the survey is an example of a data quality issue. In order to make appropriate plots or summaries of the variable ``pets``, we will need to *clean* our dataset.

If you have time, see if you can figure out what this code does on your own.

```{r, eval=F, warning=F}
form_data <- form_data %>% mutate(pets.numeric = as.numeric(pets))
form_data_subset <- form_data %>% filter(!is.na(pets.numeric))
```

In this case, the data are small enough that we could also *manually* clean our dataset as follows. We first identify the observations that are causing us problems. 

```{r}
which(is.na(form_data$pets.numeric))
form_data$pets[15]
form_data$pets[36]
```

We then update their values manually.
...
Finally, we still have to *tell* R that we are sure that ``pets`` is a number now. This time, I will just override the original variable ``pets``, instead of making a new one called ``pets.numeric``.

```{r, eval=F, warning=F}
form_data <- form_data %>% mutate(pets = as.numeric(pets))
```

# Summarize a single quantitative variable
...
Play around with the code below. Is there a histogram that you like best? Do you feel that ``binwidth`` matters for a histogram?
...
I am assuming that you are all familiar with the concepts of mean, median, and mode. We can compute these all easily in R! Usually, the name of the relevant ``R`` function is often just the english name of the statistic we want to compute. If it is not, google can easily help you out.
...
Try running the same lines of code as above, but change the word ``mean()`` to the word ``median()``. What happens? For this dataset, do you prefer the mean or the median? Why?
...
Confusingly, ``mode()`` does not give you the mode of a numerical variable. It tells you the *type* of variable that R thinks this variable is. We would need to do a bit of wrangling to obtain the mode: my favorite way is to look at a table of the variable, and just observe here that the mode is 2 pets.

```{r}
mode(form_data$pets)
table(form_data$pets)
```
...
Are you familiar with these various measures of spread? If not, don't worry! We will revisit IQR and standard deviation next week!
...
The formatting and some of the introductory content in this tutorial was adapted from an OpenIntro lab.
