...
- Load our first dataset into R, and perform basic exploratory data analysis.
</div>
...
```{r}
form_data <- read.csv("https://anna-neufeld.github.io/tutorials/Week1/form_data_anonymous.csv")
```
...
Last year, many people added commentary to their numerical answers. For example, these were all answers that I got for "How many pets have you had or have you lived with in your life?" on this survey last year. 
...
There are a lot more topics that we could cover in data cleaning; but we will come back to these later in the semester. For now, our goal is to visually and numerically describe variables!
...
Let's first learn how to make a histogram in R. There are actually two ways. The first may look simpler, but I would like to encourage you all to use the second version. 

```{r}
hist(form_data$pets)
ggplot(data=form_data, aes(x=pets))+geom_histogram()
```
I actually sort of prefer the *look* of ``hist()`` to the look of ``ggplot()``. However, ``ggplot()`` turns out to be a really powerful tool for data visualization. It is going to be easy to customize our plots, add extra variables, add legends and captions, etc. Thus, I encourage you to get familiar with ``ggplot``.
